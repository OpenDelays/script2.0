...
include 'connect.php';
    for ($m = 0; $m < $nbf; $m++)
		...
		{echo "QUERY EFFETTIVE - $q<br>";$q++;mysql_query($query) or  die ("ERRORE : ".mysql_error());}
		}
}
function pTime()
	{
...
function timeStampToDelay($ts,$ts2)
		{		
        $delay=($ts2/1000-$ts/1000)/60; // divide for millisecond then secon to obtain minutes of delays
        return $delay;
		}
...
{   
    ...
    echo "NUMERO STAZIONI ".count($lines)." - ULTIMA STAZIONE $lines[count($lines)-1]";
...
{ 
	...
	global $word_list;
	global $train_id;
    $l_data=array(array(),array(),array(),array(),array(),array());$nb_res=0;$dbdate=1;$ts=0; 
    

	for ($i = 0; $i < count($l_stations); $i++)//  ATTIVARE DOPO TEST !!!!!!!!!!!!!
	//for ($i = 0; $i < 10; $i++)
	{
    echo  "$i ";
    ...
    $nb_res = count($obj);
        ...
        if ($nb_res != 0)
		{
            ...
            if ($obj[$nb_res-1]["fermata"]["programmata"]!==null)
				...
				for ($j = 0; $j < $nb_res; $j++)
				{
				if (($obj[$j]["fermata"]["programmata"]!==null) AND ($obj[$nb_res-1]["fermata"]["effettiva"]!==null)) 
					{
